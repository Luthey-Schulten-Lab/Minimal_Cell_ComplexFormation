...
def translationWithCost(sim, sim_properties, locusNum, aasequence):
    ...
    elif locusNum in sim_properties['locations_ptns']['secreted']:
...
def addRibosomeBiogenesis(sim, sim_properties, NSpecies):
    ...
    bindingRates = pd.read_excel(sim_properties['ribo_assembly_path'], sheet_name='LSU parameters', index_col=None)
    
    assemblyRxns = pd.read_excel(sim_properties['ribo_assembly_path'], sheet_name='LSU reactions', index_col=None)
...
def getRNAInitCount(sim_properties, locusNum):
    ...
    if type == 'protein':
        ...
        mRNAcount = pd.read_excel(sim_properties['mRNA_count_path'], sheet_name = 'Sheet1')
